...
//                mCalendarProperties.getOnDayClickListener().onDayClick(eventDay);
    ...
    private void reverseUnselectedColor(SelectedDay selectedDay) {
        boolean enabledDay = mCalendarProperties.getFilledDays().contains(selectedDay.getCalendar())
                || !isBetweenMinAndMax(selectedDay.getCalendar());
        if (enabledDay) {
            DayColorsUtils.setDayColors((TextView) selectedDay.getView(), mCalendarProperties.getDaysLabelsColor(),
                    Typeface.NORMAL, R.drawable.background_color_circle_stroke);
//            AAAAAAAAAAAAAAAAAAAAAAAAAA
            return;
        }
//        mCalendarProperties.getFilledDays().contains(eventDay.getCalendar())
...
//        jfhghjkhk
    ...
    private void callOnClickListener(EventDay eventDay) {
        boolean enabledDay = mCalendarProperties.getDisabledDays().contains(eventDay.getCalendar())
                || !isBetweenMinAndMax(eventDay.getCalendar());
        boolean filledDay = mCalendarProperties.getFilledDays().contains(eventDay.getCalendar())
                || !isBetweenMinAndMax(eventDay.getCalendar());
        if (filledDay) eventDay.setFilled(filledDay);
        else eventDay.setEnabled(enabledDay);
        mCalendarProperties.getOnDayClickListener().onDayClick(eventDay);
    }
}
